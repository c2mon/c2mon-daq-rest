...
/**
 * This is a helper Class to coordinate all functionality of this DAQ. There
 * are two different ways to trigger Actions. The first is to use the
 * {@link IDataTagChanger} to add, update and remove DataTags of this DAQ. The
 * second way comes through the MessageHandler itself, which can trigger a
 * refresh of all DataTags of this daq.
 * <p/>
 */
public class RequestDelegator {
   ...
   * In order to do this the manager determines which kind of functionality
   * (GET or POST) these DataTags belong to.Furthermore a HardwareAddress based
   * on this functionality will be created and the tag will be attached to the
   * correct scheduler.
   *
   ...
   * If the DataTag belongs to the {@link GetScheduler} a new get request will
   * be triggered. If it belongs to the {@link PostScheduler} the last
   * received value will be send to the server.
   *
